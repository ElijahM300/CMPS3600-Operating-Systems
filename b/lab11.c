 ...
 * lab11.c
 * 
 *   $ gcc -o lab11 -lm lab11.c
 *   $ ./lab11 poem log         # write 25 chars from offset 5 
 * 
 ...
 *
 * Note: mmap allows multiple processes to share virtual memory; unlike SysV 
...
int main(int argc, char *argv[])
{
	...
	fd = open(argv[1], O_RDONLY);  /* open the input file for reading */
	if (fd == -1)
		perror("open");

	if (fstat(fd, &sb) == -1)     /* returns file size in struct stat sb */
		perror("fstat");

    outf = open(argv[2], O_WRONLY|O_CREAT|O_TRUNC, 0666);
    if(outf == -1)
        perror("open");
    ...
    char *dest = mmap(NULL, sb.st_size, PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, outf, 0);
