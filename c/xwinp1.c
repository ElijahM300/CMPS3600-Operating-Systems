/* Elijah Morris
   xwinp1.c copied from xwinp.c
   modifications: added a thread to move a line of text
...
void *moveText(void *ptr) {
    ...
    move = (int *)ptr; 
    while(1) {
        /*If the t key is pressed, move the text*/
        if(*move == 1) {
            g.text_pos_x += (5 * g.direction);
            if(g.text_pos_x > 140) {
...
int main(int argc, char *argv[], char *env[])
{
    ...
    */
    ...
    if(!g.child) {
        ...
        /*Setup the ipckey and shared memory*/
        key_t ipckey;
        char pathname[128];
        ...
        /*Create the thread*/
        if(pthread_create(&text, NULL, moveText, (void *)&g.move_text) < 0) {
...
void make_new_window() {
    ...
    if(cpid == 0) {
        ...
        char *newargs[] = {"xwinp1","_CHILD_",shm, NULL};
        execve("./xwinp1", newargs, my_env);
    }
    else {
        //parent process
    }
...
void render(void)
{
    ...
    if(!g.child) {
        drawString("Thread #1", g.text_pos_x, 20);
        drawString("Press 'T' to move the text.", 10, 40);
    }
}
