/* Elijah Morris
   xwinp2.c copied from xwinp1.c
   modifications: added a second thread to control a second line of text
...
void *moveText(void *ptr) {
    ...
    printf("%i\n", *move);
    while(1) {
        /*If the t key has been pressed, the flag to move is 1, and the thread is 0, move text*/
        if((g.move_text == 1 && g.thread1 == 1) && *move == 0) {
            ...
            if(g.text1_pos_x < 10) {
                ...
                g.thread1 = 0;
                g.thread2 = 1;
                printf("thread %i moved this\n", *move);
            }
        }
        /*If the t key has been pressed, the flag to move is 1, and the thread is 1, move text*/
        else if((g.move_text == 1 && g.thread2 == 1) && *move == 1) {
...
int main(int argc, char *argv[], char *env[])
{
    ...
    */
    ...
    if(!g.child) {
        ...
        /*Setup the ipckey and initialize shared memory*/
        key_t ipckey;
        char pathname[128];
...
void make_new_window() {
    ...
    if(cpid == 0) {
        ...
        char *newargs[] = {"xwinp2","_CHILD_",shm, NULL};
        execve("./xwinp2", newargs, my_env);
    }
    else {
        //parent process
    }
