...
void *moveText(void *ptr) {
    ...
    while(1) {
        ...
        else if((g.move_text == 1 && sem_value2 == 1) && *move == 1) {
        ...
        else if((g.move_text == 1 && sem_value1 == 1) && *move == 2) {
            g.text3_pos_x += (5 * g.direction);       
            if(g.text2_pos_x > 140) {
                g.direction = -g.direction;
            }
            if(g.text2_pos_x < 10) {
...
int main(int argc, char *argv[], char *env[])
{
    ...
    */
    ...
    if(!g.child) {
        ...
        sem_value1 = semctl(semid, 0, GETVAL);
        sem_value2 = semctl(semid, 1, GETVAL);
        ...
        if(pthread_create(&text3, NULL, moveText, (void *)&move3) < 0) {
            perror("pthread_create");
            exit(EXIT_FAILURE);
        }
    }
...
void render(void)
{
    ...
    if(!g.child) {
        drawString("Press 'T' to move the text.", 10, 20);
        drawString("Thread #1", g.text1_pos_x, g.text1_pos_y);
        drawString("Thread #2", g.text2_pos_x, g.text2_pos_y);
        drawString("Thread #3", g.text3_pos_x, g.text3_pos_y);
    }
}
